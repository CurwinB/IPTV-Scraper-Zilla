...
def files(repo,path):
 url=f'https://api.github.com/repos/{repo}/contents/{path}?ref=master'; r=requests.get(url,headers=GH,timeout=30); r.raise_for_status(); items=r.json(); out=[]
 for x in items:
  if x.get('type')=='file' and re.search(r'\.(m3u8?|txt)$',x.get('name',''),re.I): out.append(x['download_url'])
  elif x.get('type')=='dir': out+=files(repo,x['path'])
 return out

def load(url):
 try:
  r=requests.get(url,headers={'User-Agent':UA},timeout=30); r.raise_for_status(); return parse(r.text,url)
...
def main():
 playlist_urls=[]
 # Both repositories are sources. IPTV's stream playlists live under streams/; Zilla is scanned recursively.
 for repo,path in [('CurwinB/iptv','streams'),('CurwinB/IPTV-Scraper-Zilla','')]:
  try:
   found=files(repo,path); print(f'{repo}: {len(found)} playlists found'); playlist_urls+=found
  except Exception as e: print(f'{repo}: discovery failed: {e}')
 ...
 channels={}
 for r in healthy:
  name=re.sub(r'\s+',' ',r.get('name') or r.get('id') or 'Unknown').strip(); key=(re.sub(r'[^a-z0-9]+','',name.lower()),r.get('country','').lower(),r.get('language','').lower())
